...
class BiLSTM_ATT(nn.Module):
    ...
    def __init__(self, conf: Config, vocab_size: int, pos_size: int, tag_size: int):
        ...
        self.att_w = nn.Parameter(torch.randn(self.batch, 1, self.hidden_dim, device=self.device))
    ...
    def attention(self, lstm_out: torch.Tensor):
        ...
        # 2. att_w: [4, 1, 200]-->[4, 200, 1]
        self.att_w = torch.transpose(self.att_w, 1, 2)
		
        # 3. 计算注意力得分 [4, 70, 200] @ [4, 200, 1] → [4, 70, 1]
        score = torch.bmm(score, self.att_w)

        # 4. 归一化权重 [4, 70, 1]
        alpha = F.softmax(score, dim=1)
		
        # 5. lstm_out: [4, 70, 200]-->[4, 200, 70]
        lstm_out = torch.transpose(self.lstm_out, 1, 2)
        
        
        # 4. 计算注意力结果 [4, 200, 70] @ [4, 70, 1] → [4, 200, 1]
