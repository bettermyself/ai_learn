...
> **基础散点图的局限性**：当数据规模较大时，传统散点图会出现严重的点重叠问题，难以准确反映数据分布密度。此时推荐采用以下两种优化方案：

#### 1.1.2 散点图优化方案

| 方案类型       | 实现方法                        | 适用场景             |
| :------------- | :------------------------------ | :------------------- |
| **随机采样法** | `df.sample(100).plot.scatter()` | 快速预览数据趋势     |
| **蜂巢密度图** | `df.plot.hexbin(gridsize=15)`   | 大数据集密度分布分析 |
...
```python
# hexbin绘制蜂巢图  gridsize参数控制蜂巢网格密度，值越大网格越细。
...
| 图表类型       | 适用场景           | 核心优势           | 是否需要透视表 |
| :------------- | :----------------- | :----------------- | :------------- |
| **散点图**     | 双变量关系探索     | 直观展示变量关联   | ❌ 否           |
| **蜂巢图**     | 大数据集密度分布   | 避免重叠，揭示热点 | ❌ 否           |
| **堆叠柱状图** | 分类数据组成对比   | 展示部分与整体关系 | ✅ 是           |
| **堆叠面积图** | 趋势变化与组成分析 | 同时展示趋势和占比 | ✅ 是           |
| **多折线图**   | 多类别趋势对比     | 清晰展示变化差异   | ✅ 是           |

**⚠️ 关键原则**：当维度复杂度≥3或需要数据聚合时，必须先构建透视表。
...
| **数据表示** | 连续平滑曲线       | 离散柱状统计       |
| **Y轴含义**  | 概率密度           | 样本数量           |
| **多类对比** | 曲线叠加，清晰易读 | 堆叠显示，容易混淆 |
| **平滑程度** | 通过核函数平滑处理 | 原始数据分箱统计   |
| **适用场景** | 观察连续概率分布   | 查看具体数值频数   |
