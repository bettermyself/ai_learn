...
数据处理在数据科学工作流程中承担两大核心职能：

- **数据分析支撑**：通过数据清洗、转换、聚合等操作，为分析提供高质量、结构化的数据基础
- **特征工程实现**：为机器学习建模准备特征，包括特征构造、分箱、归一化等关键预处理步骤
...
| **数据类型**  | **核心属性**                                    | **常用方法**                               | **适用场景**     |
| :------------ | :---------------------------------------------- | :----------------------------------------- | :--------------- |
| **Series**    | `dtype`, `index`, `name`, `values`              | `value_counts()`, `describe()`, `apply()`  | 一维数据分析     |
| **DataFrame** | `shape`, `columns`, `index`, `dtypes`, `values` | `info()`, `head()`, `tail()`, `describe()` | 二维表格数据处理 |
...
| **操作类型** | **输入**       | **输出**             | **典型应用**                 | **核心方法**            |
| :----------- | :------------- | :------------------- | :--------------------------- | :---------------------- |
| **聚合**     | 每个分组       | 每组一行（汇总统计） | 计算分组统计量（总和、均值） | `groupby().agg()`       |
| **转换**     | 分组内每行数据 | 与原数据行数相同     | 组内标准化、填充缺失值、排名 | `groupby().transform()` |
| **过滤**     | 每个分组       | 符合条件的组所有行   | 删除小样本组、按条件筛选分组 | `groupby().filter()`    |
