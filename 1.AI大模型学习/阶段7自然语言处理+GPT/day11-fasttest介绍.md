...
> ---
>
> 在 FastText 的层次 Softmax 结构中，哈夫曼树中不同标签的**权重（Weight）**主要是根据该标签在训练数据中的**出现频率（Occurrence Frequency）**来确定的。
>
> 具体确定方式及逻辑如下：
>
> 1. **权重的定义**：在构建哈夫曼树时，每一个标签（叶子节点）都会被分配一个权值。根据来源资料，这个权值直接对应于标签在语料库中出现的**频率**。
> 2. **构建原则**：
>
>   ◦ **高频标签离根节点更近**：权值越大（即出现频率越高）的标签，在树结构中距离根节点越近。
>
>   ◦ **低频标签离根节点更远**：权值越小的节点，则会被安排在树的更深层。
>
> 3. **确定权重的步骤**：
>
>   ◦ 在训练开始前，系统会统计数据集中每个标签出现的总次数。
>
>   ◦ 以这些频率作为初始权值，每次选择权值最小的两个节点进行合并，生成一个新节点，新节点的权值为两个子节点权值之和。
>
>   ◦ 重复此过程，直到所有标签都被合并到一棵树中。
>
> 4. **为什么要用频率作为权重？**
>
>   ◦ **优化计算效率**：由于层次 Softmax 将分类任务转化为了从根节点到叶子节点的路径预测，路径越短，所需的二分类（逻辑回归）计算次数就越少。
>
>   ◦ **提升整体速度**：通过将高频标签放在靠近根部的位置，可以让模型在处理最常见的类别时以极快的速度（较短的路径）完成预测，从而使整个模型的平均计算复杂度达到 *O*(log*L*)。
>
> **示例说明**： 在来源资料提供的示例中，假设有 A、B、C、D 四个标签，如果标签 B 的出现频率最高，那么在构建哈夫曼树后，标签 B 的编码最短（如编码为 `0`），这意味着预测标签 B 只需要进行一次二分类计算
...
$$
有了损失函数之后, 接下来就是对其中的$X$，$θ$进行求导，并更新。参数 *X*：隐层向量（输入向量）；参数 *θ*：内部节点的权重参数
...
>>> model = fasttext.train_supervised(
        ...
        autotuneDuration=600  # 超参数搜索的“限时（以秒为单位）
    )
```
