...
class SoftAttention(nn.Module):
    """
    软注意力机制实现（拼接+线性变换版本）
    支持三维张量输入 (batch_size, seq_len, hidden_dim)
    """
    
    def __init__(self, query_size, key_size, value_size1, value_size2, output_size):
        """
        参数说明：
        - query_size: Query向量维度
        - key_size: Key向量维度
        - value_size1: 注意力权重维度（需与value_size1匹配）
        - value_size2: Value向量第二维度
        - output_size: 最终输出维度
        """
        super().__init__()
        
        # 注意力权重计算层：将Q和K拼接后映射到value_size1维度
        self.atten = nn.Linear(query_size + key_size, value_size1)
        
        # 输出线性层：将Q与注意力结果拼接后映射到目标输出维度
        self.linear = nn.Linear(query_size + value_size2, output_size)

    def forward(self, Q, K, V):
        """
        前向传播流程：
        1. Q与K拼接 → 线性变换 → Softmax → 注意力权重
        2. 权重与V批量矩阵乘法 → 上下文向量
        3. Q与上下文向量拼接
        4. 线性变换 → 最终输出
        """
        # Step 1: 计算注意力权重 [batch, seq_len, value_size1]
        # 维度变化: Q[1,1,32] + K[1,1,32] → cat[1,1,64] → Linear → [1,1,32]
        atten_weight = F.softmax(self.atten(torch.cat((Q, K), dim=-1)), dim=-1)
        
        # Step 2: 加权求和得到上下文向量 [batch, 1, value_size2]
        # 维度变化: atten_weight[1,1,32] × V[1,32,64] → bmm → [1,1,64]
        context = torch.bmm(atten_weight, V)
        
        # Step 3: 将Query与上下文向量拼接 [batch, 1, query_size+value_size2]
        # 维度变化: Q[1,1,32] + context[1,1,64] → cat → [1,1,96]
        combined = torch.cat((Q, context), dim=-1)
        
        # Step 4: 线性变换到输出维度 [batch, 1, output_size]
...
if __name__ == '__main__':
    # 模拟单样本、单步、32维的Query和Key，以及32×64的Value矩阵
    Q = torch.randn(1, 1, 32)      # Query向量 [batch=1, seq_len=1, dim=32]
    K = torch.randn(1, 1, 32)      # Key向量  [batch=1, seq_len=1, dim=32]
    V = torch.randn(1, 32, 64)     # Value向量 [batch=1, value_size1=32, value_size2=64]
    
    # 实例化注意力模块
    attention = SoftAttention(
        query_size=32, key_size=32,
        value_size1=32, value_size2=64,
        output_size=32
    )
    
    # 前向传播，得到输出和注意力权重
    result, weights = attention(Q, K, V)
    print(f"输出维度: {result.shape}")     # torch.Size([1, 1, 32])
    print(f"权重维度: {weights.shape}")    # torch.Size([1, 1, 32])
```
