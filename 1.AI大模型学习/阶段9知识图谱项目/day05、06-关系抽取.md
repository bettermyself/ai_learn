...
- **文件路径**：`/home/ec2-user/Bilstm_Attention_RE/predict.py`

```python
from model.bilstm_atten import *      # 导入BiLSTM-Attention模型架构
from utils.data_loader import *       # 导入数据加载工具函数
from utils.process import *           # 导入数据预处理函数
import torch                          # 导入PyTorch深度学习框架
from tqdm import tqdm                 # 导入进度条显示工具

# 导入配置文件，包含超参数和路径设置
conf = Config()

# 获取词汇表映射：word到id的转换字典
# 这是将文本转换为模型可处理的数字序列的关键
word2id, id2word = get_word_id(conf.train_data_path)
vocab_size = len(word2id)  # 词汇表大小，决定模型的嵌入层维度

# 位置信息的最大范围（句子中实体相对位置的最大距离）
pos_size = 143

# 关系类别数量，决定模型输出层的维度
tag_size = len(relation2id)
print(f"关系类别数量: {tag_size}")  # 打印关系类别总数

# 创建id到关系名称的反向映射，用于将模型预测结果转换为可读标签
# 示例输出：{0: "主演", 1: "作曲", ...}
id2relation = {int(value): key for key, value in relation2id.items()}

# 加载测试数据集，返回迭代器
# test_iter包含sentence, pos1, pos2, label等字段
_, test_iter = get_loader_data()

# 实例化BiLSTM-Attention模型并移动到指定设备（CPU/GPU）
# 参数：配置对象、词汇表大小、位置范围、关系类别数
ba_model = BiLSTM_ATT(conf, vocab_size, pos_size, tag_size).to(conf.device)

# 加载训练好的模型权重文件
# 注：原代码单引号不匹配，已修正
ba_model.load_state_dict(torch.load("./save_model/new_model_40.bin"))
...
def model2predict():
    ...
    # 将模型设置为评估模式
    # 关键作用：禁用Dropout和BatchNorm的训练行为，确保预测稳定性
    ba_model.eval()
    
    # 禁用梯度计算，减少内存消耗并加速推理
    with torch.no_grad():
        # 遍历测试数据迭代器
        # 每个batch包含：句子、实体1位置、实体2位置、真实标签、原始序列、原始标签、实体列表
        for sentence, pos1, pos2, label, original_sequences, original_labels, entities in test_iter:
            
            
            # 将数据输入模型，获取预测输出
            # output形状: [batch_size, tag_size]，表示每个样本属于各关系的概率
            output = ba_model(sentence, pos1, pos2)
            
            # 实现模型的预测：获取概率最大的关系类别ID
            # torch.argmax: 在tag_size维度上找最大值索引
            # tolist(): 将tensor转换为Python列表
            predict_ids = torch.argmax(output, dim=1).tolist()
            
            # 遍历当前batch的每个样本
            for i in range(len(original_sequences)):
                # 将token列表拼接为完整句子字符串（中文需无缝拼接）
                ...
                # 打印预测结果对比
                print('=' * 80)  # 分隔线
                print(f'原始句子：{original_sequence}')
                print(f'真实关系类别：{original_label}')
                print(f'实体列表：{entity}')
                print(f'模型预测的关系类别：{predict_label}')
                print('*' * 80)  # 结束分隔线
...
**💡 技巧**：使用 `torch.no_grad()` 上下文管理器是推理阶段的**最佳实践**，可显著减少内存占用（约减少50%）并提升约20%的计算速度。



**预测结果展示**

以下为模型在测试集上的实际预测效果，展示三组成果：

| 序号 | 原始句子                                                     | 实体对                     | 真实关系 | 预测结果 | 是否正确 |
| :--- | :----------------------------------------------------------- | :------------------------- | :------- | :------- | :------- |
| 1    | 飞虎是中国香港电视广播有限公司制作的时装警匪电视剧，由马德钟、宣萱领衔主演，林志华监制，制作成本逾千万港元 | ['飞虎', '宣萱']           | 主演     | 主演     | ✅        |
| 2    | 飞虎是中国香港电视广播有限公司制作的时装警匪电视剧，由马德钟、宣萱领衔主演，林志华监制，制作成本逾千万港元 | ['飞虎', '马德钟']         | 主演     | 主演     | ✅        |
| 3    | 王铮亮除了操刀负责该剧的片头曲、插曲以及配乐在内的所有制作，更创作了片尾曲《只是没有如果》 | ['只是没有如果', '王铮亮'] | 作曲     | 作曲     | ✅        |
