...
torch.sigmoid(x).sum().backward()  # 这里使用 sum() 是因为 backward() 只能对标量（单个数值）进行自动求导。torch.sigmoid(x) 返回的是一个向量（张量），不能直接调用 backward()。通过 .sum() 把所有元素加起来变成一个标量，这样才能进行反向传播，计算 x.grad。这里sum()的作用是把1000个sigmoid输出值加起来得到一个标量，求和操作不影响每个独立元素的梯度计算。然后backward()计算这个标量相对于原始1000个输入值的梯度，结果存储在x.grad中。
...
- `y_pred` 必须为 **float32**（logits）
- `y_true` 必须为 **int64**（类别索引）
- **禁止**在输出层手动添加Softmax，否则导致重复计算
...
- `y_pred` 和 `y_true` **都必须**为 **float32**
- 与`CrossEntropyLoss`不同，`BCELoss`**不包含**Sigmoid，需手动添加
- 等价实现：`nn.BCEWithLogitsLoss()` = Sigmoid + BCELoss（数值更稳定）
...
- **梯度不光滑**：在原点不可导，导致优化时可能“跳过”极小值（在左右横跳）。
...
💡 **核心思想**：AdaGrad 为不同参数分量自适应调整学习率——**梯度大的维度学习率小，梯度小的学习率大**；整体学习率随训练逐渐减小。
...
> ⚠️ **致命缺陷**：累积梯度平方和单调递增，学习率过早衰减至接近0，导致训练停滞。
...
💡 **核心思想**：融合Momentum和RMSProp优势，并加入偏差修正，是目前最稳定、最高效的通用优化器。
...
# scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[20,60,90,135,180], gamma=0.8)
...
**核心思想**：对每个mini-batch的数据进行标准化，使分布稳定，加速训练。
...
class Model(nn.Module):
    ...
    def forward(self,x):
        ...
        x2 = self.layer2(x1_s)
        x2_s = torch.relu(x2)
        out =self.layer3(x2_s)  # 一定不要加softmax
        return out
