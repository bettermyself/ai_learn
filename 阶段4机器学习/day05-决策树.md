...
- 每个非叶节点只有两个分支（二叉树结构）
- Cart回归树使用平方误差最小化策略作为特征选择标准，Cart分类生成树采用的基尼指数最小化策略作为特征选择标准
- 能够处理连续型和离散型特征
...
- **single/divorced分支**(样本1,3,5,7,8,10)：
  - 违约：3 yes, 3 no
  - Gini = $1 - (3/6)^2 - (3/6)^2 = 0.5$

- **基尼指数**：
  $\frac{4}{10}×0 + \frac{6}{10}×0.5 = 0.3$
...
对连续特征，CART会：
1. 将值排序：60,70,75,85,90,95,100,120,125,220
2. 计算相邻中点：65,72.5,80,87.5,92.5,97.5,110,122.5,172.5
3. 对每个分割点计算基尼指数
