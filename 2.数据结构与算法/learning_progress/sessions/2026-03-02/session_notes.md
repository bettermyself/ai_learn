...
### LeetCode 143 重排链表复习（4个问答全部正确）

**问题1**：重排链表的"三步走"分别是什么？每一步的核心操作是什么？

**回答**：
- 快慢指针找中点：慢指针走1步，快指针走2步，终止时慢指针在中点
- 反转链表：通过 prev, curr, temp 三个变量逐步反转
- 交替合并：通过两个变量保存两个链表的下一个节点，逐步拼接

✅ 正确

---

**问题2**：假设链表有 5 个节点 `1→2→3→4→5`，快慢指针找中点后，slow 停在哪个节点？如果是 4 个节点呢？

**回答**：
- 5 个节点：slow 停在节点3
- 4 个节点：slow 停在节点3（中间偏右）

✅ 正确

---

**问题3**：在反转链表时，我们需要哪三个指针？移动顺序是什么？
...
**问题4**：交替合并时，为什么循环条件是 `while prev and head`，而不是 `while prev`？循环结束时 A 链表剩余节点如何处理？

**回答**：
- `while prev` 也可以，因为 B 链表长度 ≤ A 链表
- 用 `and` 从健壮性角度更好，能处理边界情况
- 循环结束后 A 剩余节点不需要处理（最多剩1个节点，已通过 `prev.next=temp_A` 连接）
