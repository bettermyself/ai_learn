...
**问题1**：链表 `1 → 2 → 3 → 4 → 5`，删除倒数第 3 个节点，fast 先走 3 步后位置？同步前进终止时 slow 位置？
- **回答**：fast 先走3步后，fast 在节点3，slow 在 dummy；同步前进后 fast 在节点5，slow 在节点2
- **验证**：✅ 正确，slow 最终在节点2，`slow.next` 就是待删除的节点3

**问题2**：为什么用 `while fast.next` 而不是 `while fast`？
- **回答**：用 `while fast` 会导致 fast 走到 None，slow 多走一步到待删除节点本身，无法执行删除（需要前驱节点）
- **验证**：✅ 正确，深刻理解了需要前驱节点才能删除

**问题3**：链表 `[1, 2]`，删除倒数第 2 个节点（头节点），虚拟头节点的作用？
- **回答**：虚拟头节点作为头节点的前驱节点，让删除操作统一化
- **验证**：✅ 正确

**问题4**：时间复杂度和空间复杂度？
- **回答**：时间 O(n)，fast 只遍历一次；空间 O(1)
- **验证**：✅ 正确

**复习结论**：Day 33 知识点完全巩固，4个问答全部正确
