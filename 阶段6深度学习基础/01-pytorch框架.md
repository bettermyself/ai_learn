...
| 维度         | 传统机器学习                     | 深度学习                         |
| :----------- | :------------------------------- | :------------------------------- |
| **特征工程** | 依赖人工设计特征，需专业领域知识 | 端到端自动提取特征，无需人工干预 |
| **模型结构** | 特征工程与预测模型分离           | 特征提取与预测由同一个网络来完成 |
...
- **高精度**：大数据场景下表现卓越
- **强拟合**：可逼近任意非线性关系
- **工具成熟**：TensorFlow、PyTorch等框架开箱即用

#### ⚠️缺点

- **黑箱问题**：可解释性差，决策逻辑不透明
- **调参复杂**：网络结构、学习率等超参数众多 
- **数据饥渴**：依赖海量数据，训练耗时且算力要求高
- **易过拟合**：小数据集表现差，需正则化/数据增强
...
| 场景           | 共享内存                | 不共享内存                                                   |
| -------------- | ----------------------- | ------------------------------------------------------------ |
| Tensor → NumPy | `data_tensor.numpy()`   | `data_tensor.numpy().copy()`                                 |
| NumPy → Tensor | `torch.from_numpy(arr)` | `torch.tensor(arr) \ torch.from_numpy(data_np.copy())`       |
| 张量的元素类型 | -                       | `data_tensor.type(torch.DoubleTensor)` 或 `data_tensor.double()` |
...
| /          | `torch.div(a,b)` 或`a/b`  | `a.div(b)`             | `a.div_(b)`            |
...
> `data[[[1],[3]],[1,3]]` **广播机制讲解**：
>
> ##### 1. 形状分析 (Shape Analysis)
>
> 首先看这两个索引的形状：
>
> - **行索引 `idx1`**: `[[1], [2]]` $\rightarrow$ 形状是 **$(2, 1)$**
> - **列索引 `idx2`**: `[2, 3]`     $\rightarrow$ 形状是 **$(2,)$**（在计算时会自动视为 **$(1, 2)$** 以匹配维度）
>
> ##### 2. 广播过程 (The Broadcasting Step)
>
> 为了让这两个索引能够“配对”，系统会将它们广播成一个共同的形状：**$(2, 2)$**。
>
> - 行索引广播 $(2, 1) \rightarrow (2, 2)$:
>
>   列向复制，把唯一的一列复制两份。
>
>   ```
...
> ##### 3. 最终坐标配对 (Pairing)
>
> 现在两个索引形状一致了，系统会将对应位置的元素组合成坐标：
>
> - 左上角：行 `1`, 列 `2` $\rightarrow$ **$(1, 2)$**
> - 右上角：行 `1`, 列 `3` $\rightarrow$ **$(1, 3)$**
> - 左下角：行 `2`, 列 `2` $\rightarrow$ **$(2, 2)$**
> - 右下角：行 `2`, 列 `3` $\rightarrow$ **$(2, 3)$**
>
> **最终结果：** 得到了一个 $2 \times 2$ 的小矩阵，包含了原图中这四个位置的元素。

> <font color='red'>`,`区分行和列，但是要具体看数据的维度，来确定哪个`,`才是区分点。</font>

> 要理解`data[[1,2],[3,4]]`为什么取 (1,3) 和 (2,4) 这两个位置的元素。
>
> 对于 `data[index1, index2]`：
>
> - 如果 `index1` 和 `index2` 是**切片**（如 `1:3`），结果是**矩形子块**。
> - 如果 `index1` 和 `index2` 是**列表/Tensor**，结果是**离散点的集合**（点的坐标由列表元素对应产生）。
>
> ##### 对比：为什么不是取“矩形区域”？
>
> 如果你想要取第 1、2 行与第 3、4 列交叉出的 **4 个元素**（即一个 $2\times2$ 的小矩阵），你应该使用以下两种方法之一：
>
> - 切片法（适用于连续区间）：`data[1:3, 3:5]`
> - 两次索引法（适用于不连续区间）：`data[[1, 2]][:, [3, 4]]`这里先取出第 1、2 行，再在结果中取出第 3、4 列。
...
def test01():
    ...
    # 反向传播
    loss.backward()  # 计算∂loss/∂w和∂loss/∂b
