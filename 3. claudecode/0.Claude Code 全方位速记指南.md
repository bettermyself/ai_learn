...
- **定义**：主 Claude (编排者) 可以委托任务的进程，它们拥有有限的权限范围。子代理可以在后台或前台运行，从而为主代理释放上下文资源。
- **特点**：
  - 范围有限，可后台或前台运行。
  - 可以独立使用特定的 Skills。
  - 支持沙盒化，配置允许的工具、MCP 和权限，以实现正确的范围界定。
- **结构示例** (`~/.claude/agents/`):
...
- **作用**：连接外部服务（如数据库、部署平台），它并非 API 的替代品，而是对 API 的一种提示驱动式封装，从而在信息导航方面提供了更大的灵活性。
- **示例**：
- **例如：** Supabase MCP 允许 Claude 提取特定数据，并直接向上游运行 SQL，无需复制粘贴。数据库、部署平台等也同样适用。

  [![Supabase MCP 列表表](assets/04-supabase-mcp.jpeg)](https://github.com/affaan-m/everything-claude-code/blob/main/assets/images/shortform/04-supabase-mcp.jpeg) *Supabase MCP 列出公共模式中的表的示例*
...
- **关键警告 (CRITICAL)**：**上下文窗口管理**。
  ...
  - **黄金法则**：配置中保留 20-30 个 MCP，但启用的 MCP 不超过 10 个，激活的 MCP 不超过 80 个。
  - 管理命令：`/plugins` 或 `/mcp`。
