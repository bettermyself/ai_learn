...
> **数据洞察**：当数据集规模较大时，传统散点图难以准确反映数据分布密度，此时可采用两种优化方案
...
# 筛选出前5品类的葡萄酒数据 isin Series中的元素, 是否在列表范围内,如果在返回True ,不在返回False
...
```python
# 创建数据透视表并绘制堆叠柱状图
wine_pivot = top_5_wine.pivot_table(
    index='points',
    columns=['variety'],
    values=['description'],
    aggfunc='count'
...
`stacked=True` 默认是**False**（ False的情况下柱子不会堆叠起来），设置为True之后会有堆叠的效果
...
| 图表类型   | 适用场景           | 优势                   |
| :--------- | :----------------- | :--------------------- |
| 散点图     | 双变量关系探索     | 直观展示变量间关联     |
| 蜂巢图     | 大数据集密度分布   | 避免重叠，揭示分布热点 |
| 堆叠柱状图 | 分类数据组成对比   | 展示部分与整体关系     |
| 堆叠面积图 | 趋势变化与组成分析 | 同时展示趋势和占比     |
| 多折线图   | 多类别趋势对比     | 清晰展示变化趋势和差异 |



**需要先构建透视表的图形**

以下图形通常需要先创建**透视表（pivot_table）**，因为它们涉及多维度数据的聚合：

| 图形类型         | 原因说明                                             | 示例场景                       |
| :--------------- | :--------------------------------------------------- | :----------------------------- |
| **堆叠柱状图**   | 需要按行列维度聚合数据，展示多个类别在子分类上的分布 | 不同葡萄酒品种在各评分段的分布 |
| **堆叠面积图**   | 需要计算每个分类在连续轴上的累计值                   | 多品类葡萄酒评分趋势变化       |
| **多系列折线图** | 需要为每个类别生成独立的系列数据                     | 前5品种的评分随时间变化趋势    |
...
以下图形可直接使用原始数据字段生成，无需预先聚合：

| 图形类型       | 原因说明                          | 示例场景                   |
| :------------- | :-------------------------------- | :------------------------- |
| **散点图**     | 直接映射两个连续变量到坐标轴      | 价格与评分的关系           |
| **蜂巢图**     | 自动计算二维空间中的点密度        | 价格-评分的密度分布        |
| **简单折线图** | 单变量趋势分析（x轴需为有序数据） | 某品种葡萄酒随时间评分变化 |
| **直方图**     | 自动进行分箱统计                  | 评分分布                   |
| **箱线图**     | 自动计算统计量（四分位/离群值）   | 不同价格区间的评分分布     |

```py
# 直接字段使用示例
df.plot.scatter(x='price', y='points')
df.plot.hexbin(x='price', y='points', gridsize=15)
```
...
> **提示**：当不确定时，先用`df.plot()`尝试直接绘图。若图形不符合预期（如多条线重叠），再考虑构建透视表。
...
| 特点             | 说明                                        |
| :--------------- | :------------------------------------------ |
| **简化复杂图表** | 用更少代码实现分面图、分布图等复杂图表      |
| **内置统计功能** | 自动添加分布拟合、误差线、回归线等统计元素  |
| **美观默认主题** | 提供 `darkgrid`、`whitegrid` 等多种专业主题 |
| **Pandas集成**   | 直接使用DataFrame字段名绘图，无需数据转换   |
...
| 参数     | 作用                | 示例                      |
| :------- | :------------------ | :------------------------ |
| `data`   | 指定DataFrame数据源 | `data=tips`               |
| `x`, `y` | 指定X/Y轴数据列     | `x='total_bill', y='tip'` |
| `hue`    | 按类别列分组着色    | `hue='sex'`               |
| `ax`     | 指定绘图坐标轴      | `ax=axes[0,0]`            |
...
| 场景         | 方法                          | 示例                                          |
| :----------- | :---------------------------- | :-------------------------------------------- |
| **通用设置** | `plt.figure(figsize=(w,h))`   | `plt.figure(figsize=(10,6))`                  |
| **子图场景** | `plt.subplots(figsize=(w,h))` | `fig, ax = plt.subplots(figsize=(8,4))`       |
| **分面图**   | `height`+`aspect`参数         | `sns.catplot(height=4, aspect=2)`             |
| **全局默认** | `sns.set_theme()`             | `sns.set_theme(rc={'figure.figsize':(12,8)})` |
...
| 对比维度       | KDE 图                   | 直方图                       |
| :------------- | :----------------------- | :--------------------------- |
| **数据表示**   | 连续平滑的概率密度曲线   | 离散区间的柱状统计           |
| **Y轴含义**    | 概率密度                 | 样本数量/频数                |
| **多类对比**   | 曲线叠加，对比清晰       | 堆叠显示，容易混淆           |
| **适用场景**   | 观察连续数据的概率分布   | 查看离散数据的频数分布       |
| **平滑程度**   | 通过核函数平滑处理       | 原始数据分箱统计             |
| **可视化效果** | 更适合展示趋势和分布形态 | 更适合展示具体数值和区间分布 |

**核心区别：**

- KDE 图的 Y 轴表示概率密度（曲线下面积为1）
- 直方图的 Y 轴表示实际样本数量或频数
- 当需要对比多个类别分布时，KDE 图的多条曲线比直方图的堆叠柱更易读
...
| 图表       | 函数              | 适用场景             |
| :--------- | :---------------- | :------------------- |
| 直方图     | `sns.histplot()`  | 查看数据分布和频数   |
| KDE图      | `sns.kdeplot()`   | 观察连续数据概率密度 |
| 计数柱状图 | `sns.countplot()` | 分类变量频数统计     |
...
#### 2.2.4 barplot
 ...
 barplot 是对 x 分组 , 对y聚合, 聚合函数 通过`estimator`传入, 默认是mean 平均
...
| 图表           | 函数                        | 特点               |
| :------------- | :-------------------------- | :----------------- |
| 散点图         | `sns.scatterplot()`         | 基础关系探索       |
| 带回归线散点图 | `sns.regplot()`             | 展示线性趋势       |
| 蜂巢图         | `sns.jointplot(kind='hex')` | 大数据集密度分布   |
| 2D KDE图       | `sns.kdeplot(x,y)`          | 双变量联合密度分布 |
| 条形图         | `sns.barplot()`             | 分类变量聚合比较   |
| 箱线图         | `sns.boxplot()`             | 分布与异常值检测   |
| 提琴图         | `sns.violinplot()`          | 分布密度可视化     |
...
| **场景类**型 | 适用条件                  | 典型案例         |
| ------------ | ------------------------- | ---------------- |
| **标准RFM**  | 有消费金额(M)且频率(F)高  | 电商/打车/旅游   |
| **RFE变体**  | 无消费金额但用户活跃度高  | 社交/内容平台    |
|              | （E=Engagement 互动行为） | 点赞/评论/分享等 |
...
- **数据读取与格式：**
  - ✅ 所有工作表数据均成功读取，**无任何错误**。
  - ✅ “提交日期”列已被自动识别为**日期格式**，后续分析无需转换。
- **订单金额分布特征：**
  - ⚠️ 订单金额分布**极不均匀**，存在显著的**极大值**。
  - **示例 (2016年数据)：**
    - 最大值：**174,900**
    - 最小值：**0.1**
  - **影响：** 极大值与极小值相差悬殊，数据整体将受到**极值显著影响**。
- **异常订单金额识别与处理方案：**
  - ⚠️ 存在极低值订单（如 **0、0.1 元**），经与业务方沟通确认为**非正常订单**。
  - **原因分析：**
    - **高值订单 (如 174,900)：** 确认为**有效订单**，通常对应客户一次性购买多个高价值商品（如大家电）。
    - **低值订单 (<1元，如 0.1)：** 主要为**使用优惠券支付的订单**，其金额**无实际业务分析意义**。
  - **处理决定：**
    - ➡️ **所有低于 1 元的订单**均存在此问题。
    - ➡️ 在后续数据处理与分析中，**需剔除所有低于 1 元的订单记录**。
