...
- **正确答案**: B、D
- **结果**: ❌ 不正确
- **问题**: 混淆了引用类型（Object、Array）和原始类型
- **纠正**: Object、Array、Function 都是引用类型

---

### 问题 2：null vs undefined

**题目**: 请用自己的话解释 `null` 和 `undefined` 的区别。

- **学生回答**: undefined是变量声明但未赋值（被动），null是主动设置为空
- **结果**: ✅ 正确
- **评价**: 核心理解到位
...
```javascript
console.log(1 + "2" + "2");   // 正确: "122", 学生: 5 ❌
console.log(1 + +"2");        // 正确: 3, 学生: 3 ✅
console.log(1 + -"1");        // 正确: 0, 学生: 11 ❌
console.log(+"1" + +"1");     // 正确: 2, 学生: 2 ✅
console.log([] == 0);         // 正确: true, 学生: true ✅
console.log([] === 0);        // 正确: false, 学生: false ✅
```

**解析**:
- `1 + "2" + "2"` → `"122"`（从左到右：`1+"2"="12"`, `"12"+"2"="122"`）
- `1 + -"1"` → `0`（`-"1"=-1`, `1+(-1)=0`）
...
- ✅ **通过**
- **优势**: 一元运算符理解正确，== vs === 掌握良好
- **需加强**: 字符串拼接从左到右执行顺序，一元减号的理解

### 下次复习重点

1. 字符串 + 数字的混合运算（尤其是连续多个+）
2. 一元减号 `-"1"` 的转换规则
