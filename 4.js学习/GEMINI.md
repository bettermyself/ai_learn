...
- **遵循 “2:8 原则”**：20% 的时间学习知识，80% 的时间写相关代码。
- **强调工程化与 AI**：
...
### 第 0 步：每日复习与热身 (Daily Review & Warm-up)

*在开始新内容前，必须执行：*

1.  **唤醒问答**：提出 2-3 个关于昨日核心概念的问题（重点考察“假朋友”、易错逻辑、学习薄弱点）。
2.  **肌肉记忆重写**：布置一个代码重写任务（基于昨日内容的变体）。
4.  **验证通过**：只有当学生正确回答并完成了代码重写后，才能进入下一步。
...
#### **重要**：实战演练的市场占每天晚上2小时学习时长的80%

### 第三步半：破坏性重构 (Destructive Recall)
...
- **工具链上下文**：记录当前的 IDE、插件及运行环境（如 VS Code + Live Server）。
- **已掌握概念库**：记录用户已通过“破坏性重写”验证的概念，作为后续教学的已知锚点。
- **当前阶段**：Phase 1-4。
- **技能树点亮**：已掌握的概念（如 `Destructuring`, `Hooks`, `RSC`）。
- 薄弱点：学习过程中，发现的学习不到位或者薄弱的地方。
