...
- 在线性模型中加入 **L1 范数惩罚项** 会得到 **稀疏解**（sparse solution）。
- 结果：大部分特征的权重被压缩为 **0**，仅保留少数有效特征。
- 因此，L1 正则化天然具备「特征选择」能力。
...
# ---------- 3. 训练 L1 正则化 SVM ----------
# dual=False：当样本数 > 特征数时推荐
l1_svc = LinearSVC(C=0.01, penalty="l1", dual=False).fit(X, y)

# ---------- 4. 使用 SelectFromModel 提取非零特征 ----------
selector = SelectFromModel(l1_svc, prefit=True)
X_new = selector.transform(X)
print("筛选后维度:", X_new.shape)    # (150, 3)
```

✅ 实践小结

- **无需手动实现**：只要在线性模型（`LinearSVC`, `LogisticRegression`, `Lasso` 等）里使用 `penalty='l1'`，即可自动完成特征选择。
- **配合 `SelectFromModel`**：可直接拿到「非零权重」特征子集，无缝接入下游模型。
...
> **释义：**对内部的数据特征进行监控。
>
> **目标：**保证上线前后特征的「稳定性」与「区分度」

| 阶段                   | 监控维度   | 指标 & 阈值                                 | 解读与动作                                                   |
| ---------------------- | ---------- | ------------------------------------------- | ------------------------------------------------------------ |
| **前端监控**（授信前） | 特征稳定性 | **缺失率**、**零值率**、**PSI**             | • 缺失率陡升（如 Week3 28%）→ 排查采集/传输问题<br>• PSI > 0.1 → 预警并观察 |
| **后端监控**（放款后） | 特征区分度 | **AUC/KS 波动 ≤ ±10%**<br>**IV** 容忍度略高 | • KS ≥ 0.2（线上 A 卡合格线）<br>• IV 波动大通常因分箱变化，可容忍 |
...
| 指标                        | 容忍范围 | 异常动作     |
| --------------------------- | -------- | ------------ |
| 各箱 **bad\_rate** 小幅波动 | 可接受   | 持续观察     |
| 箱间 **风险顺序反转**       | 不可接受 | 立即迭代特征 |
...
> **目标：**在「覆盖度 / 区分度 / 稳定性」之间做权衡，并避免数据泄露

1️⃣ 评估维度与阈值

| 维度       | 定义 & 计算                 | 经验阈值                                |
| ---------- | --------------------------- | --------------------------------------- |
| **覆盖度** | `交集用户数 ÷ 内部目标客群` | 视成本而定，付费数据需 ≥ 业务可接受下限 |
| **区分度** | AUC / KS / IV / 风险趋势    | AUC 提升 2~3 个点即有价值               |
| **稳定性** | PSI（同内部特征）           | PSI ≤ 0.1 为佳                          |

2️⃣ 实操 checklist

- **避免未来信息**
  - 只用最近一个周期建模
  - 用 3~4 个月前的历史数据回测，确认无衰减
- **防止内部信息泄露**
  - 对外匹配前，先做 **Hash-ID**（不可逆），匹配上的是共有的数据，匹配不上的外部无法得知其身份
- **防止第三方美化结果**
  - 对外送样时 **掺假样本**（假用户/假标签），内部再过滤后计算真实覆盖度
...
> - **Precision** = TP / (TP + FP)  → 命中名单里坏客户占比
> - **Recall**    = TP / (TP + FN)  → 坏客户被名单捕获的比例
> - **F1**        = 2·P·R / (P + R)

> 目标：Precision 高（节省成本）、Recall 高（减少漏杀）
...
![image-20250721162131689](assets\day04\image-20250721162131689.png)
