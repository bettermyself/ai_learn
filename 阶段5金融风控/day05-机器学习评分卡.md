...
> 标准模型：逻辑回归，集成学习（可解释性强）

- **人工审核机制**

  - **高分段/低分段**：模型表现好，人工审核需求低

  - **中间分段**：需人工介入

  - **长期趋势**：业务稳定后人工审核逐步减少但**不会完全取消**

- **策略调整规范**
  - **规则（策略）与模型调整必须错开**，避免同时修改（控制变量法）
...
| 卡类型          | 观察期（X变量）                           | 表现期（Y标签）   | 典型场景   |
| --------------- | ----------------------------------------- | ----------------- | ---------- |
| **A卡（实时）** | 申请时点前6~12个月                        | FPD7/FPD30        | 新客申请   |
| **白名单A卡**   | 邀请/激活时点前6~12个月                   | FPD30             | 白名单用户 |
| **B卡**         | 未逾期用户用信前6~12个月                  | 后续2~6期DPD30/60 | 老客未逾期 |
| **C卡**         | 逾期用户还款日后1天/30天/60天往前6~12个月 | 当期DPD30/60/90   | 逾期老客   |



**还款状态与DPD分类**（还款状态和DPD一起刻画了用户的逾期情况）

| 还款阶段     | 完全还款     | 部分还款         | 展期                       | 未还款     |
| ------------ | ------------ | ---------------- | -------------------------- | ---------- |
| **还款日前** | FB（好用户） | PB               | EB（提前展期）             | NB         |
| **还款日后** | FA（催回）   | PA（意愿但无力） | EA（违约后展期，可能高危） | NA（高危） |
...
- **主流模型**：机器学习（逻辑回归/集成学习），深度学习在探索中
- **优先级**：可解释性 > 稳定性（PSI） > 区分度（AUC/KS）
...
| 指标类型       | 定义与应用场景                                               |
| -------------- | ------------------------------------------------------------ |
| AUC，KS        | AUC反映模型区分度的**平均**状况<br />KS反映了模型区分度的**最佳**状况 |
| PSI            | PSI（与特征PSI一致）                                         |
| 通过率，逾期率 | 通过率 vs 逾期率平衡(逾期率控制在比较合理的范围的前提下，要提高通过率)：<br>- A卡：优先保证通过率，容忍部分逾期<br>- B卡：优先降低逾期率，提升好用户额度 |

> 总结：AUC和KS
>
> <img src="assets/day05/roc1.png" alt="image-20200913215515056" style="zoom:50%;" />
>
> - **AUC**：ROC曲线下面积，反映整体排序能力
> - **KS**：max(TPR - FPR)，反映最佳区分阈值
...
| **步骤** | **关键动作**                | **输出/目标**                  | **备注**             |
| -------- | --------------------------- | ------------------------------ | -------------------- |
| 1        | **整理样本**                | 清洗后的训练/测试数据集        | 确保数据质量与一致性 |
| 2        | **全特征训练**              | 基线模型（初步表现评估）       | 使用所有可用特征     |
| 3        | **特征筛选**                | 降维后的特征子集               | 剔除冗余/低贡献特征  |
| 4        | **模型调参**                | 优化后的超参数组合             | 网格搜索/贝叶斯优化  |
| 5        | **进行实验**                | 新旧模型对比报告               | A/B测试或离线回溯    |
| 6        | **按实验结论再训练**        | 修正后的模型版本               | 根据实验结果调整策略 |
| 7        | **精细化特征筛选与调参**    | 最终特征集 + 最优参数          | 迭代至收敛           |
| 8        | **Backtest（历史验证）**    | 历史数据上的稳定性与区分度报告 | 验证时序外推能力     |
| 9        | **Forwardtest（未来验证）** | 未来数据上的实际业务指标       | 上线前最终评估       |
...
- **逻辑回归输出**：概率值 `p ∈ (0,1)`，表示用户违约概率
- **实际应用痛点**：
  - 若两用户概率为 `0.61` vs `0.62`，如何决策？
  - **解决方案**：将概率映射到 **350-950分** 的标准化评分区间（如芝麻信用分）。
...
- **基础分（BaseScore）**：650分（可调）
- **步长（PDO, Point of Double Odds）**：50分（每翻倍一次非逾期概率，加50分）

**示例映射**：

| 非逾期概率 vs 逾期概率 | 分数计算                | 最终评分 |
| ---------------------- | ----------------------- | -------- |
| 2:1                    | 650 + 50×log₂(2) = +50  | **700**  |
| 4:1                    | 650 + 50×log₂(4) = +100 | **750**  |
| 8:1                    | 650 + 50×log₂(8) = +150 | **800**  |
...
**逻辑回归方程**：
$$
p = \frac{1}{1+e^{-t}}, \quad t = w_1x_1 + w_2x_2 + \dots + w_nx_n.
$$
- **推导关键步骤**：

$$
 \ln \left( \frac{p}{1 - p} \right) = t \implies \log_2 \left( \frac{P_{\text{非逾期}}}{P_{\text{逾通}} } \right) = \frac{t}{\ln 2} 
$$

- **最终评分计算：**

$$
Score = 650 + 50 \cdot \frac{w_1 x_1 + w_2 x_2 + \cdots + w_n x_n}{\ln 2}
$$
...
| 参数          | 作用                            | 典型调整场景            |
| ------------- | ------------------------------- | ----------------------- |
| **BaseScore** | 整体评分偏移（如650→600）       | 降低准入门槛            |
| **PDO**       | 分数对概率变化的敏感度（50→30） | 收紧风控（每50分→30分） |
...
- **内部数据**：
  ...
  - `act_info`
- **时间字段**：`obs_mth`
  - 表示申请日期所在月份的最后一天，数据已统一处理为当月最后一天。
...
> ✅ **建议**：使用最后一个月（`2018-11-30`）作为时间外样本（Out-of-Time Sample）。
